...
class UnitsRepository extends AbstractEntityRepository {
  async create(data, options) {
    data = new Units().cast(data);
    const namHienTai = moment().year();

    const record = {
      id: this.newId(),
      unitsTitle: data.unitsTitle,
      productStall: data.productStall,
      2019: {
        quy01: 0,
        quy02: 0,
        quy03: 0,
  ...
  async update(id, data, options) {
    ...
    const record2 = {
      id,
      unitsTitle: data.unitsTitle || null,
      productStall: data.productStall || null,
    };
    ...
    if (
      ...
      await AbstractEntityRepository.executeOrAddToBatch(
        ...
        admin
          .firestore()
          .doc(
            `${new Units().collectionName}/${record.id}`,
          ),
        record,
        options,
      );
      ...
      if (data.year === moment().year()) {
        await AbstractEntityRepository.executeOrAddToBatch(
          ...
          admin
            .firestore()
            .doc(
              `${new Units().collectionName}/${record3.id}`,
            ),
          record3,
          options,
        );
      ...
      await AbstractEntityRepository.executeOrAddToBatch(
        ...
        admin
          .firestore()
          .doc(
            `${new Units().collectionName}/${record2.id}`,
          ),
        record2,
        options,
      );
      ...
      await this.refreshTwoWayRelations(record2, options);
      return record2;
    }
  }
  ...
  async populate(record) {
    ...
    // record.quy01 =
    //   (await this.findRelation('units', record.quy01)) ||
    //   {};
