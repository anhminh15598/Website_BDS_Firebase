...
const fields = {
  ...
  doanhSo: new StringArrayField(
    'doanhSo',
    label('doanhSo'),
    {},
  ),

  productUnit: UnitsField.relationToOne(
    'productUnit',
    label('productUnit'),
    {
      required: true,
  ...
  iamTeam: StallField.relationToOne(
    'iamTeam',
    label('iamTeam'),
    {
      required: true,
  ...
  productVariation: VariationField.relationToOne(
    'productVariation',
  ...
  fullName: new StringField('fullName', label('fullName'), {
    required: true,
    matches: /[A-Za-z0-9]/,
    // matches: /[^a-z0-9A-Z_ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ]/u,
  }),
  ...
  staffDateOfBirth: new DateField(
    'staffDateOfBirth',
    label('staffDateOfBirth'),
    { required: true },
  ),
